...
export const login = (credential) => {
  const formData = new FormData(); // formData is a built-in object that allows us to send data to the server
  formData.append("username", credential.username);
  formData.append("password", credential.password);
  
    // this is request status instead of response, 
    // uses then to handle, similar to event handler, it will wait for the request to complete
  return fetch(loginUrl, { // fetch is a built-in function that allows us to send a request to the server
    method: 'POST',
    credentials: 'include',
    body: formData
  }).then((response) => {// when the request is complete, the response will be passed to the then function
